...
import net.mcreator.blahmod.item.SkylandsItem;
import net.mcreator.blahmod.block.TropicsStoneBlock;
...
public class SkylandsDimension extends BlahmodModElements.ModElement {
	@ObjectHolder("blahmod:skylands_portal")
	public static final CustomPortalBlock portal = null;
	public SkylandsDimension(BlahmodModElements instance) {
		super(instance, 88);
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				ObfuscationReflectionHelper.setPrivateValue(WorldCarver.class, WorldCarver.CAVE, new ImmutableSet.Builder<Block>()
						...
						.add(TropicsStoneBlock.block.getDefaultState().getBlock()).build(), "field_222718_j");
	...
	public void clientLoad(FMLClientSetupEvent event) {
		...
		DeferredWorkQueue.runLater(() -> {
			try {
				...
				effectsRegistry.put(new ResourceLocation("blahmod:skylands"), customEffect);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static final TicketType<BlockPos> CUSTOM_PORTAL = TicketType.create("skylands_portal", Vector3i::compareTo, 300);
	public static class POIRegisterHandler {
		...
		public void registerPointOfInterest(RegistryEvent.Register<PointOfInterestType> event) {
			poi = new PointOfInterestType("skylands_portal", Sets.newHashSet(ImmutableSet.copyOf(portal.getStateContainer().getValidStates())), 0, 1)
					.setRegistryName("skylands_portal");
	...
	public void initElements() {
		...
		elements.items.add(() -> new SkylandsItem().setRegistryName("skylands"));
	}
		...
		public void onEntityCollision(BlockState state, World world, BlockPos pos, Entity entity) {
			if (!entity.isPassenger() && !entity.isBeingRidden() && entity.isNonBoss() && !entity.world.isRemote && true) {
				if (entity.func_242280_ah()) {
					...
					entity.func_242279_ag();
					teleportToDimension(entity, pos, RegistryKey.getOrCreateKey(Registry.WORLD_KEY, new ResourceLocation("blahmod:skylands")));
				} else {
					entity.func_242279_ag();
