...
public class NetherfrostBrickSideSlabBlock extends BlahmodModElements.ModElement {
	@ObjectHolder("blahmod:netherfrost_brick_side_slab")
	public static final Block block = null;
	public NetherfrostBrickSideSlabBlock(BlahmodModElements instance) {
		super(instance, 329);
	}
	...
	public static class CustomBlock extends Block implements IWaterLoggable {
		...
		public CustomBlock() {
			super(Block.Properties.create(Material.ROCK).sound(SoundType.NETHER_BRICK).hardnessAndResistance(2f, 6f).setLightLevel(s -> 0)
					.harvestLevel(0).harvestTool(ToolType.PICKAXE).setRequiresTool().notSolid().setOpaque((bs, br, bp) -> false));
			this.setDefaultState(this.stateContainer.getBaseState().with(FACING, Direction.NORTH).with(WATERLOGGED, false));
			setRegistryName("netherfrost_brick_side_slab");
		}
		...
		public boolean isSideInvisible(BlockState state, BlockState adjacentBlockState, Direction side) {
			return false; //adjacentBlockState.getBlock() == this ? true : super.isSideInvisible(state, adjacentBlockState, side);
		}
